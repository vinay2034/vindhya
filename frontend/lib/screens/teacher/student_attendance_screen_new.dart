...
class _StudentAttendanceScreenState extends State<StudentAttendanceScreen>
  ...
  Future<void> _loadInitialData() async {
    ...
    try {
      ...
      final subjects = List<Map<String, dynamic>>.from(subjectsResponse.data['data'] ?? []);
      
      setState(() {
        _classes = classes;
        _subjects = subjects;
        
        // Create tabs for class-subject combinations
        final tabCount = _classes.length;
          ...
          children: [
            ..._classes.asMap().entries.map((entry) {
              ...
              return Padding(
                ...
                child: ChoiceChip(
                  label: Text(
                    'Grade ${classData['grade']} - ${_subjects.isNotEmpty ? _subjects[0]['name'] : 'Subject'}',
                  ),
                  selected: isSelected,
                  onSelected: (selected) {
                    if (selected) {
